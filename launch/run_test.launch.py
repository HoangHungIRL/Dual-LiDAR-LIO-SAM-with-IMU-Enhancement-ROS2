...
# --------------------------------------------------------------
# Merged launch file (everything inside the lio_sam package):
#   • LIO-SAM core + TF + RViz + scans merger
#   • imu_ekf_node  (EKF IMU filter)
#   • euler_plotter.py
# --------------------------------------------------------------
...
def generate_launch_description():
    # ------------------------------------------------------------------
    # 1. Package share directory (only lio_sam)
    # ------------------------------------------------------------------
    ...
    declare_params = DeclareLaunchArgument(
        ...
        description='Full path to the ROS 2 parameters file for LIO-SAM.'
    )
    ...
    # ---- RViz config -------------------------------------------------
    rviz_cfg = os.path.join(share_dir, 'config', 'ITone.rviz')
    ...
    declare_front_x = DeclareLaunchArgument(
        'front_x', default_value='0.0',
        description='X position of front lidar (m)'
    )
    declare_back_x = DeclareLaunchArgument(
        'back_x', default_value='-0.5',
    ...
    nodes = [
        ...
        # ---- Back lidar static TF (hard-coded as in original) --------
        Node(
            package='tf2_ros',
            executable='static_transform_publisher',
            name='back_lidar_tf',
        ...
        # ------------------------------------------------------------
        #  IMU filter (EKF) – now inside lio_sam
        # ------------------------------------------------------------
        Node(
